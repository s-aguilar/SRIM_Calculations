...
class SRIM_Table(object):
    def __init__(self,fname):
        ...
        # eV / A
        units_eVa = {'eV':1, 'keV':1e-3, 'MeV':1e-6, \
                        'm':1e-10, 'mm':1e-7, 'um':1e-4, 'A':1}

        # keV / um
        units_keVum = {'eV':1e3, 'keV':1, 'MeV':1e-3, \
                        'm':1e-6, 'mm':1e-3, 'um':1, 'A':1e4}

        # MeV / mm
        units_MeVmm = {'eV':1e6, 'keV':1e3, 'MeV':1, \
                        'm':1e-3, 'mm':1, 'um':1e3, 'A':1e7}

        # keV / (ug/cm2)
        units_keVugcm2 = {'eV':1e3, 'keV':1, 'MeV':1e-3, \
                        'm':1e-2, 'mm':1e1, 'um':1e4, 'A':1e8}

        # MeV / (mg/cm2)
        units_MeVmgcm2 = {'eV':1e6, 'keV':1e3, 'MeV':1, \
                        'm':1e-2, 'mm':1e1, 'um':1e4, 'A':1e8}

        # keV / (mg/cm2)
        units_keVmgcm2 = {'eV':1e3, 'keV':1, 'MeV':1e-3, \
                        'm':1e-2, 'mm':1e1, 'um':1e4, 'A':1e8}

        # eV / (1E15 atoms/cm^2)
        units_eVatomscm2 = {'eV':1, 'keV':1e-3, 'MeV':1e-6, \
                        'm':1e-2, 'mm':1e1, 'um':1e4, 'A':1e8}

        # L.S.S. reduced units (NO IDEa WHAT UNITS THESE ARE)
        units_lss = {}
            ...
            for line in f:
                ...
                if conv:
                    ...
                    dictConversion = {'eV / A':convert(f.readline()),
                        ...
                        'L.S.S. reduced units':convert(f.readline()) }
                    break
                ...
                data.append({'E':np.float64(E)/dictOfUnits[Eu], # Must divide to get correct conversion
                'Range':np.float64(L)/dictOfUnits[Lu],          # Must divide to get correct conversion
